...
<table class="<?php print $classes; ?>">
  ...
  <tbody>
    <?php foreach ($rows_flipped as $field_name => $row) : ?>
      <?php if($field_name == 'nid') {
      	$nid = $row[0];
      } ?>
      <tr class="<?php print $row_classes_flipped[$field_name]; ?>" <?php if($field_name == 'nid') { print 'style="display:none;"'; } ?>>
        <th class="<?php print($header_classes[$field_name]); ?>">
...
<div class="prod-container" style="display:none;">
	<?php
		$node = node_load($nid);
